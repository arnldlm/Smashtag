...
class TweetsTableViewController: UITableViewController, UITextFieldDelegate {
    ...
    @IBAction func refresh(sender: UIRefreshControl?) {
        if searchText != nil {
            ...
            request.fetchTweets { (newTweets) -> Void in // asynchronous API
                dispatch_async(dispatch_get_main_queue(), { () -> Void in // Because getting tweets is done in a seperate thread, and must get back on main thread to do the UI stuff following
    ...
    func textFieldShouldReturn(textField: UITextField) -> Bool {
        if textField == searchTextField { // makes sure we're refering to a specific textfField
            textField.resignFirstResponder() // hides keyboard after search
    ...
    private struct Storyboard {
        static let CellReuseIdentifier = "Tweet" // set in storyboard
    }
    ...
    // MARK: - Prepare for Mentions Segue
